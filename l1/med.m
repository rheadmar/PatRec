...
for i = 1:length(x1)
  for j = 1:length(y1)
    distA = ([x1(i);y1(j)]-u_A)' * ([x1(i);y1(j)]-u_A);
    distB = ([x1(i);y1(j)]-u_B)' * ([x1(i);y1(j)]-u_B);
    if distA < distB
      c1(i,j) = 1;
    else
      c1(i,j) = 2;
...
for i = 1:length(x2)
  for j = 1:length(y2)
    distC = ([x2(i);y2(j)]-u_C)' * ([x2(i);y2(j)]-u_C);
    distD = ([x2(i);y2(j)]-u_D)' * ([x2(i);y2(j)]-u_D);
    distE = ([x2(i);y2(j)]-u_E)' * ([x2(i);y2(j)]-u_E);
